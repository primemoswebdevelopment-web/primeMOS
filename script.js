...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Testimonials Collapsible Toggle for Events Page
  const toggleBtn = document.getElementById('toggle-testimonials-btn');
  const testimonialsCollapse = document.getElementById('testimonials-collapse');
  
  if (toggleBtn && testimonialsCollapse) {
    const triggerToggle = (show) => {
      const isHidden = show !== undefined ? !show : testimonialsCollapse.style.display === 'none';
      if (!isHidden) {
        testimonialsCollapse.style.display = 'block';
        setTimeout(() => {
          testimonialsCollapse.style.opacity = '1';
          // Trigger the fly-in animations on mobile / fade-lift on desktop
          const cards = testimonialsCollapse.querySelectorAll('.feature-card');
          cards.forEach(card => card.classList.add('appear'));
        }, 15);
        toggleBtn.innerHTML = '<i class="fa-solid fa-chevron-up" style="margin-right: 8px;"></i> Hide Student Reviews';
      } else {
        testimonialsCollapse.style.opacity = '0';
        setTimeout(() => {
          testimonialsCollapse.style.display = 'none';
        }, 400);
        toggleBtn.innerHTML = '<i class="fa-solid fa-chevron-down" style="margin-right: 8px;"></i> Show Student Reviews &amp; Testimonials';
      }
    };
    ...
    // If navigated via hash (#testimonials) from another page, open collapse automatically and scroll to it
    ...
    // Also override standard hash changes dynamically
    ...
    // Target any link in events-hero that links to #testimonials
    const heroReviewsLink = document.querySelector('.events-hero a[href="#testimonials"]');
    if (heroReviewsLink) {
      heroReviewsLink.addEventListener('click', (e) => {
        e.preventDefault();
        triggerToggle(true);
        setTimeout(() => {
          const testimonialsSection = document.getElementById('testimonials');
